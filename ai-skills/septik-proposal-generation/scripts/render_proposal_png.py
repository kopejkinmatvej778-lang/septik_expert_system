...
def draw_rows(draw: ImageDraw.ImageDraw, rows: list[tuple[str, str, str, str, str, str]], top: int, row_h: float, mode: str, fonts: dict[str, ImageFont.FreeTypeFont]) -> None:
    ...
    for idx, row in enumerate(rows):
        ...
        name_font = fnt_small if text_width(draw, name, fnt) > 360 else fnt
...
def render(payload: dict[str, Any], template: Path, output: Path, font_regular: str, font_bold: str) -> dict[str, Any]:
    warnings: list[dict[str, Any]] = []
    proposal = payload.get("payload", payload)
    client = proposal.get("client", {})
    materials = normalize_rows(proposal.get("materials", []))
    works = normalize_rows(proposal.get("works", []))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render Septik Expert proposal PNG from JSON payload.")
    parser.add_argument("--payload", required=True, type=Path)
    parser.add_argument("--template", required=True, type=Path)
    parser.add_argument("--output", required=True, type=Path)
    parser.add_argument("--font-regular", default="/System/Library/Fonts/Supplemental/Times New Roman.ttf")
    parser.add_argument("--font-bold", default="/System/Library/Fonts/Supplemental/Times New Roman Bold.ttf")
    args = parser.parse_args()

    result = render(load_json(args.payload), args.template, args.output, args.font_regular, args.font_bold)
