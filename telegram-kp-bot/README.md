...
- опционально ищет/создает контакт и сделку в amoCRM, добавляет туда КП;
- ежедневно может проверять сделки без активных задач, писать примечания, ставить задачи и присылать отчет руководителю.
...
После генерации PNG бот:

- ищет сделку/контакт по телефону;
- если нашлась одна сделка, добавляет в нее примечание с текстом КП и ссылкой на PNG;
- если найден один контакт без сделки, создает сделку и связывает ее с контактом;
- если ничего не найдено, создает контакт и сделку с источником `Telegram bot`;
- если найдено несколько похожих сделок, не склеивает их автоматически и сообщает менеджеру в Telegram;
- записывает сумму КП в бюджет сделки amoCRM, чтобы аналитика видела выручку по сделке.

Обновление бюджета включено по умолчанию:

```bash
AMOCRM_UPDATE_LEAD_PRICE=1
```

Чтобы временно отключить обновление бюджета:

```bash
AMOCRM_UPDATE_LEAD_PRICE=0
```

## Агент сделок без задач

Ежедневный агент контроля ищет открытые сделки amoCRM без активных задач, анализирует карточку, контакты и последние примечания, добавляет контрольное примечание, ставит задачу со следующим шагом и присылает отчет руководителю.

Проверить без записи в amoCRM:

```bash
python -m septik_kp_bot.deal_task_agent --dry-run
```

Запустить с записью примечаний и задач:

```bash
python -m septik_kp_bot.deal_task_agent
```

Настройки:

```bash
AMOCRM_TASK_REPORT_CHAT_IDS=5765707209
AMOCRM_TASK_DUE_HOUR=11
```

Если `AMOCRM_TASK_REPORT_CHAT_IDS` пустой, отчет отправляется в `OWNER_TELEGRAM_USER_IDS`, если бот уже может писать этим пользователям.

На VPS включить запуск каждый день в 18:00:

```bash
sudo cp deploy/septik-deal-task-agent.service /etc/systemd/system/
sudo cp deploy/septik-deal-task-agent.timer /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now septik-deal-task-agent.timer
```
